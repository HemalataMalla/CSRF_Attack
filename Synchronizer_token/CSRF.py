...
def signin(email,username,password,profession,token):
    global Base_accno
    c=check(username)
    if c==0:
        ...
        file.write(str(Base_accno+l)+'\t'+email+'\t'+username+'\t'+password+'\t'+profession+'\t'+amount+'\t'+token+'\n')
        file.close()
        return 1
...
def index():
    ...
    if formid== 2:
        ...
        #i= len(user_details)
        #user_details.insert(i,[email,password,"1000",profession,""])
        x = signin(email,username,password,profession,token)
        if x==1:
